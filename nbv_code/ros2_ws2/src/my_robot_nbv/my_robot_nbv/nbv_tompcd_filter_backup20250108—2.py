...
class MyNode(Node): #construct Node class
    def __init__(self): #construct constructor
        ...
        # horizontal_fov =  1.480#1.5009832 # in radians 1.089
        ...
        # # self.cx = image_height / 2
        # # self.cy = image_width / 2
        ...
        # Calculate the focal length ========================
        # self.fx = 450#384.0613098144531#image_width / (2 * np.tan(horizontal_fov / 2))
        # self.fy = self.fx  # Assuming square pixels, so fx == fy

        # # Assume cx and cy are at the center of the image
        # self.cx = 380#(324.42742919921875)#image_width / 2
        # self.cy = 200#(237.085205078125) #image_height / 2
        # ======================== adjusting 20250108===================
        ...
        self.Image_subscriber_=self.create_subscription(sensor_msgs.Image, "/cam/image_raw_original", self.callback_image_visual, 10)
        ...
        # self.ready_for_next_iteration_msg
        self.is_moving_msg = False
        self.iteration_msg=1000 #這個先這樣保險一點
        self.detection_done_msg = False
            ...
            if(self.is_moving_msg==False):
                ...
                if(self.bboxReadyFlag == True): # 如果bounding box有得到東西了, 才做下面的東東 
                    ...
                    try: #街收到新data後除非transform有成功讀到, 才繼續往下做
                        ...
                        TransformStamped_before_ready=False
                        pass
                    # if(TransformStamped_before_ready==True): #好了才開始做下面 #==========================================原本的
                    if(True):
                        self.get_logger().info('IN!!') # CHANGE
                        ...
                        for point in int_data:
                            ...
                            if not math.isinf(x) and not math.isinf(y) and not math.isinf(z):
                                ...
                                # if r > 100 and g < 100 and b < 100: #==========================================原本的
    ...
    def project_point_to_pixel(self, x, y, z):#project_point_to_pixel(self, x, y, z):
        ...
        print(f'(x,y,z) is: ({str(x)},{str(y)},{str(z)})')
        ...
        # v = self.fx * (y / z) + self.cx
        # u = self.fy * (x / z) + self.cy
