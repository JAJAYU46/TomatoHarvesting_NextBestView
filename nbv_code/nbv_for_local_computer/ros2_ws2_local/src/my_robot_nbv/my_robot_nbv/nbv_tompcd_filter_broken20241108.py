...
class MyNode(Node): #construct Node class
    def __init__(self): #construct constructor
        ...
        self.static_transform = None  # To store the transform permanently
        self.static_timestamp = None  # To store the timestamp



        
    def callback1(self, msg:sensor_msgs.PointCloud2): #construct a callback
        # ================= Deal with frame change between the time before and after computing ==============
        # 處理frame的改變, 把讀入data時的frame記錄下來, 之後不管frame change成啥了, 都是用原來讀入的frame去publish的
        # try: #by TransformStamped 感覺像是把現在frame停格在這裡的感覺, 所以之後publish 到'camera_link_optical' 就會是原來那個frame
        ...
        # self.static_transform = TransformStamped.transform  # Save the transform
        # self.static_timestamp = msg.header.stamp             # Save the timestamp
        ...
        msg.header=std_msgs.Header(frame_id='camera_link_optical', stamp=msg.header.stamp)#, stamp=Clock().now().to_msg())#cam_frame_pcd(for realsense）#設定這個點雲圖會顯示到的frame ID      
        # TransformStamped
        # msg.header=std_msgs.Header(frame_id=TransformStamped.header.frame_id, stamp=TransformStamped.header.stamp)#, stamp=Clock().now().to_msg())#cam_frame_pcd(for realsense）#設定這個點雲圖會顯示到的frame ID      
        
        
        #<Debug> header 要加, stamp=msg.header.stamp 表我要拿msg.header.stamp這個時間stamp的'camera_link_optical'去當我的header!
        ...
        if filtered_points: #如果有filter到東西(is not empty)再做下面的icp跟publish
            ...
            try:
                ...
                if result_trans_final is not None: #才做下面的事情
                    ...
                    source_pointcloud2_stampheader=pc2.create_cloud(msg.header, msg.fields, source_points_np.tolist())
                    self.tompcd_ICPonly_pub_.publish(source_pointcloud2_stampheader)
                    if __debug__:
                    ...
                    # header=std_msgs.Header(frame_id='camera_link_optical', stamp=rclpy.time.Time().to_msg())
                    whole_pointcloud2 = pc2.create_cloud(msg.header, msg.fields, whole_points_np.tolist())
                    # whole_pointcloud2 = pc2.create_cloud(header, msg.fields, whole_points_np.tolist())
    ...
    def transform_pointcloud_to_np(pointcloud, transform_stamped):
        """
        ...
        source_points_np = np.array(list(pc2.read_points(pointcloud, skip_nans=True, field_names=("x", "y", "z"))))

        # Transform each point
        num_points = source_points_np.shape[0]
        points_homogeneous = np.hstack((source_points_np, np.ones((num_points, 1))))
    ...
    def convert_open3d_to_ros_pointcloud2(self, open3d_pcd, stamp):
        # Extract point cloud data
        points = np.asarray(open3d_pcd.points)
        colors = np.asarray(open3d_pcd.colors)

        header=std_msgs.Header(frame_id='camera_link_optical', stamp=stamp)#, stamp=Clock().now().to_msg())#cam_frame_pcd(for realsense）#設定這個點雲圖會顯示到的frame ID      
