...
class SdfModelV3 {
public:
    SdfModelV3(std::shared_ptr<open3d::geometry::PointCloud> pcd, float radius, int num_points)
    ...
    void ComputeSDF(std::vector<Eigen::Vector3f> query_points_input) { //Eigen::Vector3f(x, y, z)
...
private:
    ...
    void CreateRaycastingScene() { //這個做一次就好
...
{   //ROS2
	...
	// rclcpp::spin(node);
	// rclcpp::shutdown();
        // Initialize the SdfModelV3 with the file path, radius, and number of points
    ...
    SdfModelV3 processor(pcd, 0.2f, 5000); //要作為sdf 的model/ 做sample point的radius/ sample點數量



    // SdfModelV3 processor("src/dataset/data_pcd/TomatoPlant_size_modified_only1tomato_onlyRed.ply", 0.2f, 5000);//_onlyRed.ply
