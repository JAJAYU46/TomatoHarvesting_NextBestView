...
class MyNode : public rclcpp::Node
{
    ...
    private:
        ...
        {   if(finish_get_status_topic == true){ //因為一直讀不到controller topic所以一直進不去這邊 ======================
                ...
                if(octomap_done_msg_==false){ //只有false然後又接收到octomap的時候, 才要求一次nbv
                    ...
                    if(firstPCD_ready_flag==true){
                        ...
                        if (tree){
                            ...
                            if (octree){
                                ...
                                if((NbvScene.BestCandidateView_gain_percent-LastBestCandidateView_gain_percent)<0.05){ //如果新的點比舊的點少於5% 那表示這就是最終的NBV點了
