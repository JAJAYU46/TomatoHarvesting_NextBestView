        ...
        {   if(firstPCD_ready==true){
                ...
                if (tree){
                    ...
                    if (octree){
                        ...
                        std::vector<octomap::point3d> origins = {{0,0,0},{1,-0.5,0.15}};//{1,-0.5,0}
                        int id_k=0;
                        int id_t=100;
                        int id_l=200;
                        for (const auto& origin : origins) {
                            std::cout << "now origin: "<< origin << std::endl;
                            id_k++;
                            id_t++;
                            id_l++;
                            // octomap::point3d origin(0.0, 0.0, 0.0); // Define the starting point of the ray
                            ...
                            std::vector<octomap::point3d> directions = RandomPoint_Direction(origin, 2, 0.5, modelCenterOctomap_pcd); //modelCenterOctomap_pcd其實不用太長, 反正它只是個方向
                            ...
                            for (const auto& direction : directions) {
                                ...
                                // bool hit = octree->castRay(origin, direction, hitPoint);//如果是unknown point 要回傳false

                                unsigned int search_depth = 0;         // Search at full tree depth

                                OcTreeNode* result_node = octree->search(hitPoint, search_depth);

                                if (result_node != NULL) {
                                    std::cout << "Node found at specified depth!" << std::endl;
                                    std::cout << "value: " << result_node->getValue() << "\n";
                                    std::cout << hitPoint << "\n";
                                    if(octree->isNodeOccupied(result_node)){
                                        std::cout << "this node is occuppied" << "\n";
                                    }else{
                                        std::cout << "this node is free" << "\n";
                                    }
                                } else {
                                    std::cout << "Node not found!" << std::endl;
                                    std::cout << hitPoint << "\n";
                                    std::cout << "this node is unknown" << "\n";
                                }
                                ...
                                if (hit) {
                                    ...
                                    hitpointV.push_back(hitPoint);//把沒有hit的也寫入, 之後要記得刪掉這個//////////////////////////////////////////
