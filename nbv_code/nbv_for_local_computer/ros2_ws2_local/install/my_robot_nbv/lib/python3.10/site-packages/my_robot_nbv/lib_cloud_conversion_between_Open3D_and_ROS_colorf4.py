...
# Define the structured data type for the point cloud
dtype = np.dtype([
    ('x', np.float32),
    ('y', np.float32),
    ('z', np.float32),
...
def convertCloudFromOpen3dToRos(open3d_cloud, frame_id="odom"):
    # Set "header"
    header = Header()
    header.stamp = rclpy.time.Time().to_msg()#.Time.now()
    ...
    if not open3d_cloud.colors: # XYZ only
        ...
        #colors = np.floor(np.asarray(open3d_cloud.colors)*255) # nx3 matrix #open3d 的顏色是[r,g,b] 從0~1, 要先換成0~225去表示
        colors = np.floor(np.asarray(open3d_cloud.colors) * 255).astype(np.uint8)
        colors_packed_unit32 = colors[:,0] * BIT_MOVE_16 +colors[:,1] * BIT_MOVE_8 + colors[:,2]  #pack成一個unit32
        #packed_rgb = (colors_int[:, 0] << BIT_MOVE_16) + (colors_int[:, 1] << BIT_MOVE_8) + colors_int[:, 2]

        # Convert the packed uint32 into float32 (to match the format you see in ROS2 PointCloud2)
        color_packed_float32 = np.array([struct.unpack('f', struct.pack('I', int(color)))[0] for color in colors_packed_unit32])
        ...
        cloud_data = np.empty(points.shape[0], dtype=dtype)
        cloud_data['x'] = points[:, 0]
        cloud_data['y'] = points[:, 1]
        cloud_data['z'] = points[:, 2]
        cloud_data['rgb'] = color_packed_float32
